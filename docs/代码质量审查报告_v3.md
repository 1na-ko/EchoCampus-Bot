...
> ✅ **修复记录（2026-08-31）**：已修复。`ChatServiceImpl` 新增 `getOwnedConversation` 属主校验，
> `sendMessage` / `sendMessageStream`（getOrCreateConversation）均走该校验；非属主统一返回
> `CONVERSATION_NOT_FOUND` 以避免泄露会话存在性。配套单元测试见
> `ChatServiceImplTest.ConversationAuthorizationTests`。
...
> ✅ **修复记录（2026-08-31）**：已修复。`getMessages` / `deleteConversation` /
> `updateConversationTitle` 的 Service 签名均增加 `userId` 参数，Controller 透传，
> Service 层统一经 `getOwnedConversation` 校验属主。
